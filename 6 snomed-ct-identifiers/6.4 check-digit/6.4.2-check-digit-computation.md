...
The [SCTID](https://confluence.ihtsdotools.org/display/DOCGLOSS/SCTID "Glossary link: SCTID") includes a check-digit, which is generated using Verhoeff's dihedral check. This section explains the algorithm used and includes sample source code for generating and checking the check-digit in Java Script and Microsoft Visual Basic.
...
When calculating the [check-digit](https://confluence.ihtsdotools.org/display/DOCGLOSS/check-digit "Glossary link: check-digit") the same process is applied with a minor variation:

  * _Position_ is the position that the digit will have when the [check-digit](https://confluence.ihtsdotools.org/display/DOCGLOSS/check-digit "Glossary link: check-digit") has been appended.

  * The final value of _Check_ is applied to the Inverse D5 array to find the correct [check-digit](https://confluence.ihtsdotools.org/display/DOCGLOSS/check-digit "Glossary link: check-digit").

[Check-digit](https://confluence.ihtsdotools.org/display/DOCGLOSS/Check-digit "Glossary link: Check-digit")= **_ArrayInverseD5_** ( _Check_ ).
...
The ISBN modulus 11 (used for [UK NHS](https://confluence.ihtsdotools.org/display/DOCGLOSS/UK+NHS "Glossary link: UK NHS") number) picks up more errors than the IBM checksum. Leaving 2% to 3% of errors undetected. However, it generates a check-sum value of 0 to 10 and thus cannot be represented as a single [check-digit](https://confluence.ihtsdotools.org/display/DOCGLOSS/check-digit "Glossary link: check-digit") in about 9% of cases. The ISBN convention is to use "X" to represent the [check-digit](https://confluence.ihtsdotools.org/display/DOCGLOSS/check-digit "Glossary link: check-digit") value 10 but this is incompatible with an [Integer](https://confluence.ihtsdotools.org/display/DOCRELFMT/Integer+\(data+type\) "Reference term: Integer \(\(data type\)\)") representation. The [UK NHS](https://confluence.ihtsdotools.org/display/DOCGLOSS/UK+NHS "Glossary link: UK NHS") number uses this check-sum but regards and number generating a check-sum of 10 as an invalid identifier. This approach could be applied to the [SCTID](https://confluence.ihtsdotools.org/display/DOCGLOSS/SCTID "Glossary link: SCTID") but this would render 9% of possible values unusable in each partition and [namespace](https://confluence.ihtsdotools.org/display/DOCGLOSS/namespace "Glossary link: namespace"). This would prevent a simple sequence of values from being allocated as the _item identifier_ within any [namespace](https://confluence.ihtsdotools.org/display/DOCGLOSS/namespace "Glossary link: namespace"). More significantly the unusable  _item identifier_ would differ in each [namespace](https://confluence.ihtsdotools.org/display/DOCGLOSS/namespace "Glossary link: namespace") or partition and this would prevent simple transpositions of _item identifiers_ between partitions and [namespaces](https://confluence.ihtsdotools.org/display/DOCGLOSS/namespace "Glossary link: namespaces").

Partitions could be a useful way of distinguishing developmental and released components and revising the partition and recalculating the [check-digit](https://confluence.ihtsdotools.org/display/DOCGLOSS/check-digit "Glossary link: check-digit") would then be an elegant way to activate these components for a distribution version. It seems unwise to prevent future development and maintenance by using a check-sum that will prevent this.

## Verhoeff's Check

Verhoeff's check catches all single errors, all adjacent transpositions, over 95% of twin errors, over 94% of jump transpositions and jump twin errors, and most phonetic errors. Therefore, like modulus 11, the Verhoeff check reduces the undetected error rate to 2% or 3%. Unlike modulus 11, it does this using a single decimal [check-digit](https://confluence.ihtsdotools.org/display/DOCGLOSS/check-digit "Glossary link: check-digit") and without limiting the range of valid numbers.

The majority of the undetected errors with both modulus 11 and Verhoeff result from additions or omissions of digits. Any [check-digit](https://confluence.ihtsdotools.org/display/DOCGLOSS/check-digit "Glossary link: check-digit") method is likely to miss 10% of such errors and since these comprise 10% to 20%. The Verhoeff scheme also misses four jump twin errors involving digits with a difference of 5 (i.e. 050 vs. 505, 161 vs. 616, 272 vs. 727, and 494 vs. 949).
